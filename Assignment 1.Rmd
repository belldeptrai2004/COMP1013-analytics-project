...
```{r}
# Load CSV data
engines     <- read_csv("Data sets/Engine.csv")
automobile  <- read_csv("Data sets/Automobile.csv")
maintenance <- read_csv("Data sets/Maintenance.csv")
...
## Q3 — Statistical Testing and Diagnostic Relationships

```{r}
auto_full <- engines_na %>%
  left_join(automobile_na, by = "PlateNumber") %>%
  left_join(maintenance_na, by = "PlateNumber")
```
